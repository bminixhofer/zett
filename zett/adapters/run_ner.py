...
from models import RobertaWithCRF
...
def main():
    ...
    setup_adapter_training(
        model, adapter_args, data_args.dataset_name.replace("/", "_") or "ner"
    )
    ...
    if training_args.do_eval:
        ...
        metrics["avg_token_length"] = np.mean(
            [len(x) for x in eval_dataset["input_ids"]]
        )
    ...
    if training_args.do_predict:
        ...
        metrics["avg_token_length"] = np.mean(
            [len(x) for x in predict_dataset["input_ids"]]
        )
        ...
        if trainer.is_world_process_zero():
            predict_dataset.to_parquet(
                os.path.join(training_args.output_dir, "predict.parquet")
            )
